 ...
 *
 * <ol>
 *   <li><b>Module iteration.</b> Every module of the project is visited
 *   ({@link ModuleManager#getModules()}), so the same class name in different modules produces
 *   one endpoint per module.</li>
 ...
 *   framework annotation to be resolvable to real source.</li>
 *   <li><b>Framework detection.</b> A class is a controller when at least one configured resolver
 *   claims it through {@link EndpointAnnotationResolver#isController(PsiClass)}.</li>
 *   <li><b>Endpoint extraction.</b> For every method of a controller class whose
 *   {@link EndpointAnnotationResolver#methods(PsiMethod)} is not empty, one {@link ApiEndpoint} per
 *   declared HTTP method is produced, with the full path being the class base path joined to the
 *   method path with exactly one {@code /} (missing and redundant slashes are trimmed), the
 *   qualified class name as owner, the module name, and the handler {@link PsiMethod} for
 *   navigation. Endpoints whose joined path would be blank are skipped.</li>
 * </ol>
 *
 ...
 *   in dumb mode; it notifies the user and defers the scan with
 *   {@link DumbService#runWhenSmart(Runnable)} until indexing finishes.</li>
 *   <li><b>Smart-mode read action.</b> {@link #collect(Project)} runs its read action through
 *   {@link ReadAction#nonBlocking(Callable)} with {@link NonBlockingReadAction#inSmartMode(Project)},
 *   which waits for smart mode before the stub indexes are touched (and retries if indexing
 *   restarts mid-scan), instead of crashing with {@link IndexNotReadyException}.</li>
 ...
 *   only that module (logged at WARN level with its name) and keeps the endpoints of the remaining
 *   modules; a single module failure never aborts the whole scan.</li>
 * </ol>
 */
public final class ControllerEndpointSource implements EndpointSource {

    private static final Logger LOG =
            Logger.getInstance(ControllerEndpointSource.class);
    ...
    public @NotNull List<ApiEndpoint> collect(@NotNull Project project) {
        ...
        return ReadAction
                .nonBlocking((Callable<List<ApiEndpoint>>) () -> collectModules(project))
                .inSmartMode(project)
     ...
     *
     * <p>Scans the given modules one by one and accumulates their endpoints. When a module fails
     * with a {@link ProcessCanceledException}, the failure is rethrown (the platform contract
     * forbids swallowing it); when it fails with {@link IndexNotReadyException}, raised when
     * indexing starts in the middle of a scan, the failure is reported through
     * {@code onModuleFailure} and the remaining modules still contribute to the result. Any other
     * {@link RuntimeException} is not a degradation case: it propagates to fail fast on genuine
     * bugs instead of publishing a silently incomplete report.
     *
     * <p>Each module stream is materialised eagerly so the failure is attributed to its own module
     * instead of escaping lazily into a sibling's evaluation. Package-private and free of PSI
     * calls, so the degradation can be unit tested with stub scan functions.
     *
    ...
    private static void logSkippedModule(@NotNull Module module, @NotNull RuntimeException failure) {
        LOG.warn("Endpoint scan skipped the module '" + module.getName()
                + "' because its index is not ready: " + failure, failure);
    }
    ...
    private Stream<ApiEndpoint> endpointsOfNamedClass(String className, Module module, PsiShortNamesCache namesCache) {
        return Arrays.stream(namesCache.getClassesByName(className, GlobalSearchScope.moduleScope(module)))
    ...
    private Stream<ApiEndpoint> endpointsOfMethods(PsiClass psiClass, Module module,
    ...
    private Stream<ApiEndpoint> endpointsOfMethod(PsiMethod psiMethod, String basePath, String ownerClass,
            Module module, EndpointAnnotationResolver resolver) {
        String path = joinPath(basePath, resolver.methodPath(psiMethod));
        if (path.isEmpty()) {
            return Stream.empty();
        }
        return resolver.methods(psiMethod).stream()
                .map(httpMethod -> new ApiEndpoint(httpMethod, path, ownerClass, module.getName(), psiMethod));
    }

    /**
     * Joins a base path and a method path with exactly one {@code /} between them, tolerating
     * missing or redundant slashes on either side. Returns an empty string only when both inputs
     * are empty, which the caller treats as "no path declared" and skips.
     */
    private static String joinPath(String basePath, String methodPath) {
        ...
        String joined = Stream.of(base, method)
