 ...
 *
 * <ol>
 *   <li><b>Trim.</b> Surrounding whitespace is removed. An empty input yields zero segments.</li>
 *   <li><b>Strip query and fragment.</b> Everything from the first {@code ?} or {@code #} onwards
 *   is discarded: query strings never take part in the path comparison.</li>
 *   <li><b>Strip scheme and host.</b> If the input contains {@code ://} (e.g.
 *   {@code https://api.example.com/users/1}), everything up to the first {@code /} after the
 *   scheme is discarded, leaving only the path. A bare host yields zero segments.</li>
 *   <li><b>Drop host-like variable.</b> Collections frequently write
 *   {@code {{baseUrl}}/users/:id} or {@code {{ _.base_url }}/users} without a scheme, so the host
 *   appears as a leading variable segment of a path that does not start with {@code /}. When the
 *   input does not start with {@code /} and its first segment is variable-like
 *   (see rule 6), that first segment is dropped as a host placeholder. Relative paths starting
 *   with a literal segment are never touched.</li>
 *   <li><b>Split on {@code /}.</b> Empty segments are removed, which in particular ignores a
 *   trailing slash: {@code /users/} and {@code /users} are the same path.</li>
 ...
 *   <ul>
 *     <li>Handlebars / Postman: {@code {{id}}}</li>
 *     <li>Insomnia templating: {@code {{ _.id }}}, {@code {{ response.body.id }}}</li>
 *     <li>OpenAPI / JAX-RS: {@code {id}} and {@code {id:[0-9]+}} (optional regex part and
 *     optional inner whitespace)</li>
 *     <li>Rails / Sinatra / Insomnia style: {@code :id}</li>
 *   </ul>
 *   Anything else is a <b>literal</b>. The variable syntax is preserved only for display; two
 *   variables are interchangeable regardless of syntax or name.</li>
 * </ol>
 *
...
public final class PathNormalizer {
    ...
    private static final Pattern BRACES = Pattern.compile("^\\{\\s*([^{}\\s]+)\\s*(?::\\s*([^{}]*)\\s*)?}$");
    ...
    public static List<Segment> normalize(String raw) {
        ...
        boolean hostLikeVariable = !path.startsWith("/")
                && rawSegments.size() > 1
                && isVariable(rawSegments.getFirst());
        return Stream.ofNullable(hostLikeVariable ? rawSegments.subList(1, rawSegments.size()) : rawSegments)
                .flatMap(List::stream)
