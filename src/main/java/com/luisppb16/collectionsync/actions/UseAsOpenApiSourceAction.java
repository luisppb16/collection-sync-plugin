...
public final class UseAsOpenApiSourceAction extends AnAction {
  ...
  private static boolean validateOpenApiDocument(
      @NotNull File openApiFile, @NotNull Project project) {
    try {
      ...
      Messages.showErrorDialog(
          ...
          EndpointCoverageBundle.message(
              "action.openapi.unreadable.message",
              openApiFile.getName(),
              brokenDocument.getMessage()),
          EndpointCoverageBundle.message("action.openapi.invalid.title"));
      return false;
    }
  }
