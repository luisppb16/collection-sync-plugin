 ...
 * background scan.
 *
 * <p>The refresh of the table lives in {@link CoveragePanel#refresh()}; the panel runs its own scan
 * on creation, so this action only refreshes a panel that is already open. The text and description
 * come from the resource bundle via {@code withJava.xml} (key convention {@code action.<id>.text});
 * the {@code text}/{@code description} XML attributes only act as an English fallback when the
 * bundle keys are missing.
 */
public final class ScanEndpointCoverageAction extends AnAction {
  ...
  /**
   * Activates the "Endpoint Coverage" tool window and, once active, launches a background scan that
   * refreshes the open panel on completion. Shared with the context menu actions so they behave
   * exactly the same. When the tool window is not available (project without the Java plugin),
   * nothing happens.
   *
  ...
  static void activateAndScan(@NotNull Project project) {
    ...
    toolWindow.activate(
        () -> project.getService(CoverageService.class).scanAsync(() -> refreshPanel(toolWindow)));
  }
