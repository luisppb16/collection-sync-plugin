...
/**
 * "Endpoint Coverage: Check Coverage with This Collection": available on {@code .json},
 * {@code .yaml} and {@code .yml} files; validates the file as a Postman v2.1 or Insomnia v4/v5
 * collection and, when valid, adds it to the project collection paths and rescans.
 */
public final class UseAsCollectionAction extends AnAction {

    /**
     * Validates the file as a collection and, when it parses, adds its path to the settings
     * (without duplicates) and rescans with the tool window open. The settings are never touched
    ...
    private static boolean validateCollection(@NotNull File collectionFile, @NotNull Project project) {
        try {
            ...
            Messages.showErrorDialog(project,
                    EndpointCoverageBundle.message("action.collection.invalid.message",
                            collectionFile.getName(), brokenCollection.getMessage()),
