 ...
 *
 * <p>Action texts and descriptions are not read through this class: the platform resolves them from
 * the same bundle via {@code <actions resource-bundle="messages.EndpointCoverageBundle">} in {@code
 * withJava.xml}, using the {@code action.<actionId>.text} key convention. (Overriding {@code
 * getTemplateText()} is not possible: that method is {@code final} on the 2026.2 platform.) The
 * {@code actionText}/{@code actionDescription} accessors exist to resolve those same keys from
 * code, mainly for tests.
 *
 * <p>Strings are NOT externalized for: developer-facing exceptions (fail-fast messages), the
 * exported Markdown/CSV report (kept in English as a stable, tool-consumable format) and the {@code
 * plugin.xml} description/change-notes (marketplace, kept in English).
 */
public final class EndpointCoverageBundle {

  @NonNls private static final String BUNDLE_NAME = "messages.EndpointCoverageBundle";
   ...
   *
   * @param actionId action id inside the {@code collectionsync.} package, e.g. {@code
   *     collectionsync.AnalyzeEndpointsCoverage}; must not be null
  ...
  /**
   * Resolves the description of the action with the given id, using the same key the platform uses
   * for plugin.xml actions: {@code action.<actionId>.description}.
   *
   * @param actionId action id inside the {@code collectionsync.} package, e.g. {@code
   *     collectionsync.UseAsCollection}; must not be null
