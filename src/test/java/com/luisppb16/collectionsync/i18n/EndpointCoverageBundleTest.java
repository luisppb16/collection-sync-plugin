...
class EndpointCoverageBundleTest {
    ...
    @Test
    @DisplayName("Given the default and the Spanish bundle, when their keys are compared, then they are identical")
    ...
    void bundleHasNoEmptyValues(String bundleName) throws IOException {
        ...
        assertThat(bundle).allSatisfy((key, value) -> {
    ...
    @Test
    @DisplayName("Given a known key, when it is resolved, then a non-empty message with the arguments formatted is returned")
    void resolvesKnownMessageWithArguments() {
        String message = EndpointCoverageBundle.message("service.error.collection.parse", "a.json", "boom");
    ...
    @Test
    @DisplayName("Given the plugin action ids, when their texts and descriptions are resolved, then they are non-empty")
    void resolvesActionTextsAndDescriptions() {
        String[] actionIds = {"collectionsync.AnalyzeEndpointsCoverage", "collectionsync.UseAsCollection",
                "collectionsync.UseAsOpenApiSource"};

        for (String actionId : actionIds) {
            assertThat(EndpointCoverageBundle.actionText(actionId))
                    .as("action text of %s", actionId).isNotBlank();
            assertThat(EndpointCoverageBundle.actionDescription(actionId))
                    .as("action description of %s", actionId).isNotBlank();
        }
    }

    private static Properties loadBundle(String bundleName) throws IOException {
        Properties bundle = new Properties();
        try (InputStream stream = EndpointCoverageBundleTest.class.getClassLoader()
